...
public class UpsertTemplate extends LiquibaseSqlTemplate<String> {
    ...
    public UpsertTemplate(
        Database database,
        Map<ChangelogColumns, Object> replacements,
        String unescapedId
    ) {
    ...
    public String visit(ClusterConfig clusterConfig) {
        String alteredColumns = replacements.entrySet().stream()
                .map(entry -> String.format("%s = %s", entry.getKey(),
                    escape(database, entry.getValue())
                ))
    ...
    public String visit(StandaloneConfig standaloneConfig) {
        String selectedColumns =
            Arrays.stream(ChangelogColumns.values())
                .map(it -> replacements.containsKey(it) ? escape(database, replacements.get(it)) : it.name())
                .collect(Collectors.joining(", "));
        return String.format(
